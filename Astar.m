...
- (SLOT*) optimalSlot
{
	...
	for(j=(nj-1); j >= 0; j--)
		for(i=(ni-1); i >= 0; i--){
			//if( GRID[nj*i+j].status == OPEN && GRID[nj*i+j].suggested > 0)
			//	return &GRID[nj*i+j];
...
- (int) solve {
	...
	int i,loc,val,solved,oldSuggestion;
	SLOT *slot = NULL;
	...
	while([puzzle moveRemainsAt:(loc)])
	{
		...
		{
			//Don't use the suggestion more than once.
			if(slot->suggested)
				slot->suggested = 0;
	...
	//The next time through this location, 
	//A* Search will be on a different path.
	//Therefore, the suggestion made by SGA may again be valid.
	//slot->suggested = oldSuggestion;

...
/********************************************************************* 
 * PRE: none.
 * POST: A copy of self is made in a separate memory location and a 
...
- copy {
	...
	for( i=0; i < chromLen; i++ ){
		...
		newIndividual->chromosone[i].suggested = chromosone[i].suggested;		
...
- free {
	...
	for( i=0; i < chromLen; i++ )
		objc_free(chromosone[i].possibles);

	objc_free(chromosone);
...
/********************************************************************* 
 * PRE: none.
 * POST: The individual is completely initialized (Stats included).
 *********************************************************************/
...
/********************************************************************* 
 * PRE: none.
 * POST: Loaded a new chromosome and parameters.
 *********************************************************************/
...
/********************************************************************* 
 * PRE: none.
 * POST: Sets the value of an allele
 *********************************************************************/
...
/********************************************************************* 
 * PRE: none.
 * POST: Sets the value of an allele and sets the suggestion.
 *********************************************************************/
- gene:(int)loc allele:(int)val suggest:(int)sVal {
	[self gene:loc allele:val];
	chromosone[loc].suggested = sVal;
...
/********************************************************************* 
 * PRE: none.
 * POST: Open chromosome values are set to a random value
 *********************************************************************/
- randomize {
	int i;
...
/********************************************************************* 
 * PRE: none.
 * POST: Uses the values from A to suggest values for self.
 *********************************************************************/
- prefaceWith: anIndividual {
	...
	{
		if(chromosone[i].status == OPEN)
			[self gene:i allele:0 suggest:[anIndividual alleleAt:i]];
	}
...
- (int) mostLikelyAt:(int)loc {
	...
	//Check if the suggestion is possible
	//if it is, then return it as most likely...
	//This disregards the distributions...
	if(chromosone[loc].suggested)
		if(chromosone[loc].possibles[chromosone[loc].suggested-1])
		{
			//totals[ chromosone[loc].suggested-1 ]++; //Bias towards the suggestion
			return chromosone[loc].suggested;
		}
